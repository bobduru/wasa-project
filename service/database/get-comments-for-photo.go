...
func (db *appdbimpl) GetCommentsForPhoto(photoId int64) ([]Comment, error) {
	...
	query := `SELECT id, user_id, image_id, text, create_time FROM comments WHERE image_id = ?`
	...
	for rows.Next() {
		var comment Comment
		if err := rows.Scan(&comment.ID, &comment.UserID, &comment.ImageID, &comment.Text, &comment.CreateTime); err != nil {
